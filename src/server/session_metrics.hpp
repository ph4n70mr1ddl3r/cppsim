...
public:
    ...
    void update_processing_time(std::chrono::microseconds time) noexcept {
        total_processing_time_.fetch_add(time.count(), std::memory_order_relaxed);
    ...
    void update_connection_duration(std::chrono::seconds duration) noexcept {
        total_connection_time_.fetch_add(duration.count(), std::memory_order_relaxed);
    ...
    double get_average_processing_time_ms() const noexcept {
        ...
        return static_cast<double>(total) / count / 1000.0; // Convert microseconds to milliseconds
    }
...
public:
        ...
        try {
            ...
            ss << "Server Totals - Sessions: " << session_count_
               ...
               << " | Avg per session: " << (session_count_ > 0 ? 
                   static_cast<double>(total_messages_sent_) / session_count_ : 0.0) << " msgs sent";
            return ss.str();
        } catch (...) {
    ...
    [[nodiscard]] bool is_healthy() const noexcept {
        ...
        double error_rate = static_cast<double>(total_errors_) / 
                           (total_messages_sent_ + total_messages_received_);
...
private:
    ...
    void update_averages() noexcept {
        if (session_count_ > 0) {
            ...
            avg_messages_per_session_ = static_cast<double>(total_messages) / session_count_;
            
            if (total_messages > 0) {
                avg_error_rate_ = static_cast<double>(total_errors_) / total_messages;
            } else {
                avg_error_rate_ = 0.0;
            }
        } else {
